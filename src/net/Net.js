...
export default class Net {
    ...
    getOrCreateUser(callback, error = null) {
        //http://www.sweetbear.info/base/od/htdocs/index.php?m=|||uid||100200300|||s_key||-1|||m||0||0||general.get_data|||m||0||1||{}
        this.oDataLoader.request_obf('user.init', {})
            .then(callback)
            .catch(error);
    }

    getUserProfile(callback, error = null) {
        //http://www.sweetbear.info/base/od/htdocs/index.php?m=|||uid||100200300|||s_key||-1|||m||0||0||general.get_data|||m||0||1||{}
        this.oDataLoader.request_obf('user.get_profile_data', {})
            .then(callback)
            .catch(error);
    }
    ...
    storageGet(key, callback, error = null) {
        this.oDataLoader.request_obf('storage.get', {
            uid: 10541666,
            data: {
                key_data: key
            }
            ...
            .then(resp => {
                callback(resp.data[0][1]);
            })
            .catch(error);
    }

    storageSet(key, data, callback, error = null) {
        this.oDataLoader.request_obf('storage.set', {
            uid: 10541666,
            data: {
                key_data: key,
                data: data
    ...
    stageStart(stageId, callback, error = null) {
        this.oDataLoader.request_obf('mission.stage_start', {
            uid: 10541666,
            data: {
                stage_stat_id: Number(stageId),
                cost_type: 0
            }
        })
            .then(resp => {
    ...
    stageEnd(stageDynamicId, callback, error = null) {
        this.oDataLoader.request_obf('mission.stage_end', {
            uid: 10541666,
            data: {
                id: Number(stageDynamicId)
            }
        })
            .then(resp => {
                callback(resp.data[0][1]);
    ...
    missionStart(missionId, callback, error = null) {
        this.oDataLoader.request_obf('mission.start', {
            uid: 10541666,
            data: {
                mission_stat_id: Number(missionId)
            }
        })
            .then(resp => {
                callback(resp.data[0][1]);
    ...
    missionEnd(missionKey, callback, error = null) {
        this.oDataLoader.request_obf('mission.end', {
            uid: 10541666,
            data: {
                start_key: missionKey,
                point: 1
            }
        })
            .then(resp => {
    ...
    setStorage(callback, error = null) {
        this.oDataLoader.request_obf('storage.set', {}, callback, error);
    }
     ...
     * @param	callback - функция обратного вызова
     */
    puzzle_skipGame(sUid, sCatId, sLvlId, sImgId, callback, error = null) {
