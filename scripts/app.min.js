...
var GameFaq = exports.GameFaq = function GameFaq(word) {
    ...
    var imgs = [];
    var json = _app.App.assetService.getJson();

    if (_app.App.phaser.cache.checkImageKey('tutorial_0')) {
        json.tutorial.forEach(function (img, i) {
...
var Booster = exports.Booster = function Booster() {
        var _this = this;
        ...
        var booster_set_stat = _app.App.storage.generalData.shop_booster_set_stat.find(function (b) {
                return b.id == _app.App.storage.generalData.shop_booster_stat[0].id;
        });
        var booster_stat = _app.App.storage.generalData.booster_stat.find(function (b) {
                return b.id == booster_set_stat.type;
        });
        //let json = App.storage.generalData.shop_booster_set_stat.find(b => b.id == App.storage.generalData.shop_booster_stat[0].id) ;
        this.label = _app.App.phaser.add.text(4, 4, booster_stat.name, { font: "14px Arial", fill: "#000000", align: "center", wordWrap: true }, group);
        this.priceOrCount = _app.App.phaser.add.text(34, 40, '0', { font: "18px Arial", fill: "#000000", align: "center", wordWrap: true }, group);
...
var FriendItem = exports.FriendItem = function FriendItem() {
    ...
    if (data) {
        ...
        var bg = _app.App.phaser.add.image(0, 0, imgData.atlas, imgData.key);
        group.add(bg);

        this.name = _app.App.phaser.add.text(0, 78, data.nickname ? data.nickname : data.first_name + " " + data.last_name, { font: "18px Arial", fill: "#000000", align: "center" }, group);
        if (this.name.width > bg.width) this.name.width = bg.width - 4;
        this.name.x = (bg.width >> 1) - (this.name.width >> 1);

        var onComplete = function onComplete() {
            var img = _app.App.phaser.add.image(0, 0, "social_friend_" + this.fid);
            img.x = (bg.width >> 1) - (img.width >> 1);
            img.y = (bg.height >> 1) - (img.height >> 1);
            group.add(img);
        };

        _app.App.phaser.load.image("social_friend_" + data.id, data.img);
        _app.App.phaser.load.onLoadComplete.addOnce(onComplete.bind(this), this);
        _app.App.phaser.load.start();
    } else {
...
var FriendsList = exports.FriendsList = function () {
    ...
    _createClass(FriendsList, [{
        ...
        value: function fill(data) {
            ...
            var last = this.friends.children[this.friends.children.length - 1];
            var sx = this.friends.children.length > 0 ? last.x + last.width + this.space : 0;
            //пустые ячейки для друзей, вывожу хотя бы 1 в конце.
            for (var i = 0; i < (n > 0 ? n : 1); i++) {
                var f = new _friendsItem.FriendItem();
    ...
    _createClass(VKApi, [{
        ...
        value: function getFriends() {
            ...
            return new Promise(function (resolve, reject) {
                VK.api('friends.get', { fields: 'nickname,photo_50' }, function (resp) {
...
var _booster = require('../gui/boosters/booster');
...
function renderBoosters() {
	...
	var openLetter = new _booster.Booster('type0');
	cont.add(openLetter.graph);

	var removeLetter = new _booster.Booster('type1');
	removeLetter.graph.x = openLetter.graph.width + 8;
	cont.add(removeLetter.graph);

	var askFriend = new _booster.Booster('type2');
...
},{"../fotoCross/gameFaq":33,"../gui/boosters/booster":38,"../gui/btn":39,"../gui/imgBtn":43,"../gui/message":44,"../gui/moneyBtn":45,"../net/DataLoader":48,"./../app":28,"./../effects/fadeOutIn":29}],54:[function(require,module,exports){
"use strict";
...
var AssetService = exports.AssetService = function () {
    function AssetService() {
        ...
        _app.App.phaser.load.json('gdata', './data/gdata.json');
    ...
    _createClass(AssetService, [{
        ...
        value: function getJson() {
            return _app.App.phaser.cache.getJSON('gdata');
        }
    }]);
